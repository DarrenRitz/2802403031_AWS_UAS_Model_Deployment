...
if submitted:
    ...
    try:
        ...
        fin_labels = ["Annual\nIncome", "Monthly\nSalary", "Outstanding\nDebt", "Monthly\nBalance", "EMI/Month"]
        fin_values = [annual_income/1000, monthly_inhand_salary/100,
                      outstanding_debt/100, monthly_balance/100, total_emi_per_month]
        ...
        for spine in axes[0].spines.values(): spine.set_edgecolor("#1e2235")
        ...
        for spine in axes[1].spines.values(): spine.set_edgecolor("#1e2235")

        axes[1].legend(
            handles=[mpatches.Patch(color="#5c7cfa", label="≤ 5 (normal)"),
                     mpatches.Patch(color="#e74c3c", label="> 5 (tinggi)")],
            fontsize=8, facecolor="#0f1117", edgecolor="#1e2235", labelcolor="#b0b8d0"
        )
